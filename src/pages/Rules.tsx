            ...
            {bonusStructure.map((bonus, index) => (
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-base md:text-lg font-bold text-gold-300 mb-1 md:mb-2">{bonus.title}</h3>
                  <p className="text-gray-300 mb-2 md:mb-3 leading-relaxed text-xs md:text-sm">{bonus.description}</p>
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
                    <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-gold-500/20 to-amber-500/20 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                      {category.icon}
                    </div>
                    <h3 className="text-base md:text-lg font-bold text-gold-300">{category.title}</h3>
                  </div>
                  ...
                  <ul className="space-y-1 md:space-y-2">
                    {category.rules.map((rule, ruleIndex) => (
                      <li key={ruleIndex} className="flex items-start gap-3 text-gray-300">
                        <div className="w-1.5 h-1.5 bg-gold-400 rounded-full mt-2 flex-shrink-0"></div>
                        <span className="leading-relaxed text-xs md:text-sm">{rule}</span>
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 max-w-6xl mx-auto">
            {importantNotices.map((notice, index) => (
              <motion.div
                key={index}
                className="bg-gradient-to-br from-amber-500/10 to-gold-500/10 p-3 md:p-4 rounded-xl border border-amber-500/30"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
              >
                <h3 className="text-sm md:text-base font-bold text-amber-300 mb-1 md:mb-2">{notice.title}</h3>
                <p className="text-gray-300 leading-relaxed text-xs md:text-sm">{notice.content}</p>
              </motion.div>
            ))}
          </div>
        </div>
